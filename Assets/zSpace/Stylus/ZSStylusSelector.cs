...
public class ZSStylusSelector : ZSUIStylusInput
{
    ...
    [HideInInspector][SerializeField] protected ZSStylusShape _activeStylusShape;
    [HideInInspector][SerializeField] protected ZSStylusTool _activeStylusTool;
    ...
    public override Vector3 Direction { get { return transform.forward; } protected set{} }
    ...
    public ZSStylusShape activeStylus
    {
        ...
        set
        {
            if (_activeStylusShape != null)
                _activeStylusShape.gameObject.SetActiveRecursively(false);

            _activeStylusShape = value;
            _activeStylusTool = (value == null) ? null : value.GetComponent<ZSStylusTool>();

            if (_activeStylusShape != null)
                _activeStylusShape.gameObject.SetActiveRecursively(true);
        }
    }	
    ...
    protected override void OnScriptStart()
    {
        ...
        foreach (ZSStylusShape shape in GetComponentsInChildren<ZSStylusShape>(true))
        {
            ...
            shape.gameObject.SetActiveRecursively(activeStylus == shape);
        }

        GameObject centerCameraObject = GameObject.Find("ZSCenterCamera");
        if (centerCameraObject != null)
            _centerCamera = centerCameraObject.camera;
    }
    ...
    private void OnCoreUpdated(ZSCore sender)
    {
        ...
        if (_useTracking && !_zsCore.IsMouseEmulationEnabled())
        {
            ...
            if (_zsCore.IsTrackerTargetPoseValid(_targetType) && pose != _previousStylusPose)
            ...
            if (!useMousePointer)
            {
                ...
                transform.FromMatrix(pose);
                transform.localScale = _zsCore.GetWorldScale() * Vector3.one;
            }
        }
        ...
        if (_useCollision)
        {
            ...
            else
            {
                ...
                if (linearStylus != null && linearStylus._tip != null && linearStylus._tip.active)
